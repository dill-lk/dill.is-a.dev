        ...
        <div className="mb-4 w-[340px] md:w-[380px] h-[500px] bg-[var(--brand-950)]/80 backdrop-blur-2xl border border-white/10 rounded-[28px] shadow-2xl flex flex-col overflow-hidden animate-slide-up origin-bottom-right transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)]">
          ...
          <div className="h-14 px-5 flex justify-between items-center border-b border-white/5 shrink-0">
             ...
             <button 
              ...
              className="w-7 h-7 rounded-full bg-white/[0.08] hover:bg-white/20 flex items-center justify-center text-[var(--foreground)]/70 hover:text-[var(--foreground)] transition-all"
            >
              <X size={16} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-5">
            {messages.map((msg, idx) => (
              <BlurFade key={msg.id} delay={0.1 * idx}>
                <div 
          ...
          <div className="p-4 border-t border-white/5 bg-[var(--brand-950)]/50">
            <div className="relative">
              <input
                ...
                className="w-full bg-[var(--brand-600)] border border-white/10 text-[var(--foreground)] text-sm rounded-xl py-3 pl-4 pr-12 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all placeholder:text-white/40"
              />
              <button
                onClick={handleSend}
                disabled={isLoading || !inputText.trim()}
                className="absolute right-2.5 top-1/2 -translate-y-1/2 w-8 h-8 bg-blue-600 text-white rounded-lg transition-all flex items-center justify-center disabled:opacity-50 disabled:scale-90 enabled:hover:scale-105 enabled:active:scale-95 shadow-lg"
              >
